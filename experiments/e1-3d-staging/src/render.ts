...
type PreviewGeometry = THREE.BoxGeometry | THREE.SphereGeometry;
type PreviewMesh = THREE.Mesh<PreviewGeometry, THREE.MeshNormalMaterial>;
...
export function createRenderer(viewport: HTMLDivElement): RenderPort {
  ...
  return { render(state: AppState): void {
    ...
    const geometry: PreviewGeometry = state.shape === 'box'
